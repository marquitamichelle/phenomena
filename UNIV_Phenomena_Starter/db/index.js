  ...
  try {
    ...
    const { rows: comments } = await client.query (
      `SELECT * FROM comments
       WHERE 'reportId' 
       IN (${reports.map(reports => reports.id).join(',')}) `);
    ...
    reports.forEach(reports => {
      reports.comments = reports.filter(),
      reports.isExpired = Date.parse(reports.expirationDate) < new Date(),
...
async function createReport(reportFields) {
  ...
  try {
    ...
    const { rows: [ newReports] } = await client.query(`
...
async function closeReport(reportId, password) {
  try {
    ...
    if(password != report.password) {
      throw new Error('Password incorrect for this report, please try agin')
    };
...
async function createReportComment(reportId, commentFields) {
  ...
  try {
    ...
    if (!report.isOpen) {
      throw new Error('This report is not open')
    };
    ...
    if (!report.isExpired) {
      throw new Error ('This report is expired')
    }
    // all go: insert a comment
    else { await client.query (`INSERT INTO content WHERE reportId`) };
